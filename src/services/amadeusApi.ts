...
export const createFlightOrder = async (flightOffer: any, passengers: any[], email: string, phone: string) => {
  ...
  const travelers = passengers.map((p, i) => ({
    ...
    contact: {
      ...
      // ← AJOUT ICI : deviceType requis !
      deviceType: "mobile",  // ou "desktop" si tu préfères
      deviceId: `device_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`, // ID unique requis
    },
    documents: [{
      documentType: "PASSPORT",
      birthPlace: "Algeria",
      issuanceLocation: "Algeria",
  ...
  if (!response.ok) {
    ...
    throw new Error(error.errors?.[0]?.detail || 'Erreur Amadeus');
  }

  const result = await response.json();
  return result.data.associatedRecords[0].reference; // ← VRAI PNR !
};
